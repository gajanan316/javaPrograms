/*1. Sum of Two Numbers

Problem: Write a function that takes two integers and returns their sum.

Input:
a = 10, b = 20

Output:
30
*/
 /* public class SumAPP
  {
	  public static void main (String x[])
	  {
		  sum(10 , 20);
	  }
		  public static void sum(int a , int b)
		  {
			  System.out.printf("Addition of the number %d\n", a+b);
			  
		  }
	  
  }
  
  public class SumAPP
  {
	  public static void sum (int a , int b)
	  {
		  System.out.printf("Addtion of the number %d\n", a+b);
	  }
	  public static void main (String x[])
	  {
		  sum(10 , 30);
	  }
  }*/
  //or
  
  
/*
2. Check Even or Odd

Problem: Write a function that checks whether a given number is even or odd.

Input:
n = 7

Output:
Odd
*/
/*
public class OddAPP
{
	public static void odd(int a)
	{
		if(a % 2 != 0)
		{
			System.out.printf("The number is odd %d\n", a);
		}
		else
		{
			System.out.printf("number is even %d\n", a);
		}
	}
	public static void main (String x[])
	{
		odd(8);
	}
}
*/
/*
3. Find Maximum of Two Numbers

Problem: Write a function that returns the greater of two numbers.

Input:
a = 15, b = 25

Output:
25
*/
/*
 public class MaxAPP
{
	public static void max(int a , int b)
	{
		if (a > b)
		{
			System.out.printf("Max number is this %d\n", a);
		}			
		else
		{
			System.out.printf("min number is this %d\n", b);
		}
	}
	public static void main(String x[])
	{
		max (30 , 40);
	}
}
*/
/*
4. Calculate Factorial

Problem: Write a function that calculates the factorial of a number.

Input:
n = 5

Output:
120
*/
 /* public class FactoAPP
  {
	  public static void facto(int a)
	  { 
	    int fact = 1;
		  for(int i= 1 ; i<= a ; i++)
		  {
			 fact = fact * i; 
		  }
		   System.out.printf("Fcatorial of number %d\n", fact);
	  }
	 
	  public static void main (String x[])
	  {
		facto(5);
	  }
  }
  
  //////////////////////////
  public class FactorialApp {
    
    public static void main(String[] args) {
        int n = 5;
        int result = factorial(n);
        System.out.println(result);
    }

    public static int factorial(int n) {
        int fact = 1;
        for(int i = 1; i <= n; i++) {
            fact = fact * i;
        }
        return fact;
    }
}
*/
/*
5. Check Prime Number

Problem: Write a function that checks whether a number is prime.

Input:
n = 11

Output:
Prime
*/
    public class Prime
	{  
	     public static void prime(int n)
		 {
			 if(n <= 1)
			 {
				 System.out.println("not prime number ");
				 return;
			 }
		 
			 boolean PrimeNum = true ;
			 
			 for (int i= 2 ; i< n ; i++)
			 {
				 if(n % i== 0)
				 {
					 primeNum = false ;
					 break;
				 }
			 }
			 if(primeNum)
			 {
				 System.out.println("number is prime ");
			 }
			 else
			 {
				 System.out.println("number is not prime ");
			 }
	}
		 
		
		public static void main(String x[])
		{
			prime (11); // call the function
		}
	}
   
/*   
6. Reverse a Number

Problem: Write a function that reverses a given integer.

Input:
n = 1234

Output:
4321

7. Count Digits

Problem: Write a function that counts the number of digits in a given number.

Input:
n = 98765

Output:
5

8. Sum of Digits

Problem: Write a function that returns the sum of digits of a number.

Input:
n = 456

Output:
15

9. Find Square of a Number

Problem: Write a function that returns the square of a given number.

Input:
n = 9

Output:
81

10. Check Palindrome Number

Problem: Write a function that checks whether a number is a palindrome.

Input:
n = 121

Output:
Palindrome*/

